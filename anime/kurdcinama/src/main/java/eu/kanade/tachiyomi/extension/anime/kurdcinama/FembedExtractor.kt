...
/**
 * Best-effort Fembed/Membed extractor.
 *
 * This extractor fetches the embed page and searches for direct mp4/m3u8 URLs in the HTML and scripts.
 * Fembed sometimes exposes JSON with a "file" or "src" key which we try to parse.
 */
object FembedExtractor {
    ...
    fun extract(client: OkHttpClient, url: String, referer: String? = null): List<Video> {
        try {
            ...
            val body = res.body?.string() ?: return listOf(Video(url, "Fembed Embed", url))
        ...
        return listOf(Video(url, "Fembed Embed", url))
    }
